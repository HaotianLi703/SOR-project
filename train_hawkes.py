...
def train_hawkes():
 ...
 # 训练记录日志
    ...
    # compile the model
    model = model_hawkes.Hawkes(type_num=3)
    ...
    for epoch in range(epo):
        ...
        for step_train in range(data_process.max_nums['train']):
            ...
            input_list = [data_process.seq_time_to_end_numpy, 
            ...
            model.mu.data = torch.clamp(model.mu.data, min=0.0)
            model.alpha.data = torch.clamp(model.alpha.data, min=0.0)
            # 记录当前batch的数量
            my_log['iteration'] += 1
            ...
            if my_log.get('iteration') % my_log.get('track_per_batch')==0:
                ...
                for val_step in range(data_process.max_nums['dev']):
                    ...
                    input_list_val = [data_process.seq_time_to_end_numpy, 
                            data_process.seq_time_to_current_numpy, 
                            data_process.seq_type_event_numpy,
                            data_process.time_since_start_to_end_numpy,
                            data_process.seq_mask_numpy, 
                ...
                if my_log['val_tracking']['val_log_lik'][-1] > my_log['best_log_lik_val']:
                    ...
                    torch.save(model, 'model_hawkes_'+str(train_ratio)+'.pkl')
