...
def generate_sim_time_seqs(time_seqs, seqs_length):
    ...
    """
    sim_time_seqs = torch.zeros((time_seqs.size()[0], time_seqs.size()[1]-1)).float().to(device)
    sim_index_seqs = torch.zeros((time_seqs.size()[0], time_seqs.size()[1]-1)).long().to(device)
    restore_time_seqs, restore_sim_time_seqs = [], []
    for idx, time_seq in enumerate(time_seqs):
        restore_time_seq = torch.stack([torch.sum(time_seq[0:i]) for i in range(1,seqs_length[idx]+1)]).to(device)
        restore_sim_time_seq, _ = torch.sort(torch.empty(seqs_length[idx]-1).uniform_(0, restore_time_seq[-1]).to(device))
        
        sim_time_seq = torch.zeros(seqs_length[idx]-1).to(device)
        sim_index_seq = torch.zeros(seqs_length[idx]-1).long().to(device)
...
def pad_bos(batch_data, type_size):
    ...
    pad_event_seqs = torch.zeros((event_seqs.size()[0], event_seqs.size()[1]+1)).long().to(device) * type_size
    pad_time_seqs = torch.zeros((time_seqs.size()[0], event_seqs.size()[1]+1)).float().to(device)
