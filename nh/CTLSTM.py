...
class CTLSTM(nn.Module):
    ...
    def init_states(self, batch_size):
        self.h_d = torch.zeros(batch_size, self.hidden_size, dtype=torch.float).to(device)
        self.c_d = torch.zeros(batch_size, self.hidden_size, dtype=torch.float).to(device)
        self.c_bar = torch.zeros(batch_size, self.hidden_size, dtype=torch.float).to(device)
        self.c = torch.zeros(batch_size, self.hidden_size, dtype=torch.float).to(device)
    ...
    def log_likelihood(self, event_seqs, sim_time_seqs, sim_index_seqs, total_time_seqs, seqs_length, batch_first=True):
        ...
        original_loglikelihood = torch.zeros(batch_size).to(device)
        ...
        for idx, (event_seq, seq_len) in enumerate(zip(event_seqs, seqs_length)):
            original_loglikelihood[idx] = torch.sum(torch.log( 
                                                     lambda_k[idx, torch.arange(seq_len).long().to(device), event_seq[1:seq_len+1]]))
        ...
        simulated_likelihood = torch.zeros(batch_size).to(device)
        for idx, (total_time, seq_len) in enumerate(zip(total_time_seqs, seqs_length)):
            mc_coefficient = total_time / (seq_len)
            simulated_likelihood[idx] = mc_coefficient * torch.sum(torch.sum(sim_lambda_k[idx, torch.arange(seq_len).long().to(device), :]))
